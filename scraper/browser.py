"""Playwright 브라우저 세팅 — 본인 Chrome 로그인 세션을 재사용해 인스타그램 접근.

전략 (META 광고 레퍼런스 프로젝트와 동일):
1. headless=False (실제 창 띄움 — 헤드리스는 훨씬 쉽게 감지됨)
2. playwright-stealth 적용 (설치돼 있으면)
3. 본인 Chrome 프로필을 임시 위치에 복사해서 재사용 (원본 프로필과 충돌 방지)
4. 한국어/한국 로케일 강제
"""
...
def launch_context(playwright: Playwright, config: dict) -> BrowserContext:
    ...
    config 키:
      ...
      - chrome_profile_name (str, 보통 "Profile 6" 같은 이름)
      - headless (bool)
    """
    ...
    if use_profile:
        chrome_root = Path(os.path.expanduser(
            config.get("chrome_profile_path", "~/AppData/Local/Google/Chrome/User Data")
        ))
        profile_name = config.get("chrome_profile_name", "Default")
        src_profile = chrome_root / profile_name

        if not src_profile.exists():
            print(f"[경고] Chrome 프로필을 찾을 수 없습니다: {src_profile}")
            print("      -> 일반 모드로 폴백합니다 (로그인 안 된 새 세션 — 사실상 수집 불가)")
            use_profile = False
        else:
            # 안전을 위해 본인 프로필을 직접 안 쓰고 임시 복사본을 씀
            # (Chrome이 동시에 같은 프로필을 잠그면 충돌)
            tmp_profile = Path(tempfile.gettempdir()) / "influencer-fit-profile"
            ...
            context = playwright.chromium.launch_persistent_context(
                ...
                channel="chrome",  # 실제 설치된 Chrome 사용 (별도 다운로드 불필요)
                **common_kwargs,
            )
...
def _apply_stealth_to_context(context: BrowserContext) -> None:
    if not HAS_STEALTH:
        print("(playwright-stealth 미설치 — webdriver 흔적만 직접 제거합니다)")
    ...
    try:
        ...
        print(f"(stealth 적용 실패: {e} — webdriver 흔적만 직접 제거합니다)")
        context.add_init_script(
            "Object.defineProperty(navigator, 'webdriver', {get: () => undefined});"
        )
